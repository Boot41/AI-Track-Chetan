...
class QueryClassifier:
    ...
    def classify(self, message: str, session_state: SessionState | None = None) -> QueryClassification:
        ...
        if query_type in {
            ...
            requires_recomputation = not self._has_required_cache(query_type, session_state)
    ...
    def _classify_query_type(self, message: str, session_state: SessionState | None) -> QueryType:
        if any(token in message for token in ("compare", "versus", " vs ", "better investment")):
        ...
        if any(token in message for token in ("acquire", "acquisition", "license", "rights", "catalog")):
            return QueryType.ACQUISITION_EVAL
        if any(token in message for token in ("greenlight", "original", "series", "pilot", "show bible")):
            return QueryType.ORIGINAL_EVAL
        return QueryType.GENERAL_QUESTION

    def _followup_type(self, message: str, session_state: SessionState | None) -> QueryType:
        keyword_map = {
            QueryType.FOLLOWUP_WHY_NARRATIVE: ("narrative", "story", "theme", "tone", "character"),
            QueryType.FOLLOWUP_WHY_ROI: ("roi", "return", "completion", "retention", "budget"),
            QueryType.FOLLOWUP_WHY_RISK: ("risk", "contract", "rights", "regulatory", "clause"),
            QueryType.FOLLOWUP_WHY_CATALOG: ("catalog", "fit", "demographic", "strategy", "audience"),
        }
        ...
        if session_state and session_state.query_type:
            ...
            return fallback_map.get(session_state.query_type, QueryType.GENERAL_QUESTION)
        return QueryType.GENERAL_QUESTION

    def _has_required_cache(self, query_type: QueryType, session_state: SessionState | None) -> bool:
    ...
    def _looks_like_budget_scenario(self, message: str) -> bool:
        return "budget" in message and bool(
            re.search(r"\b(what if|changes? if|change|increase|increases|decrease|decreases|cut)\b", message)
        )

    def _looks_like_localization_scenario(self, message: str) -> bool:
        return any(token in message for token in ("dub", "subtit", "localization", "bahasa", "hindi"))
