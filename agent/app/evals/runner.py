...
from agent.app.evals.judge import SemanticJudge
...
from agent.app.schemas.eval_runner import (
    CaseEvalResult,
    EvalBehavior,
    EvalCase,
    EvalSuiteResult,
...
class EvalRunner:
    ...
    async def run_case(self, case: EvalCase) -> CaseEvalResult:
        ...
        # Precision@5 needs labels; using section_ids of required_evidence as proxies for relevance labels
        relevant_labels = [r.section_id for r in case.required_evidence if r.section_id]
        precision = calculate_precision_at_k(retrieved_evidence, relevant_labels if relevant_labels else None)
        ...
        for i, perturbed_config in enumerate(case.config_perturbations):
            ...
            original_outcome = orchestration_result.recommendation_result.outcome if orchestration_result.recommendation_result else None
            perturbed_outcome = perturbed_result.recommendation_result.outcome if perturbed_result.recommendation_result else None
            
            original_score = orchestration_result.recommendation_result.weighted_score if orchestration_result.recommendation_result else None
            perturbed_score = perturbed_result.recommendation_result.weighted_score if perturbed_result.recommendation_result else None
            ...
            sensitivity_results.append({
                ...
                "score_delta": (perturbed_score - original_score) if perturbed_score is not None and original_score is not None else None
            })

        # 6. Multi-turn (if sequence exists)
        multi_turn_results = []
        current_state = self.orchestrator.update_session_state(request.session_state, orchestration_result)
        for turn_query in case.multi_turn_sequence:
            ...
            multi_turn_results.append({
                "query": turn_query,
                "answer": turn_public["answer"],
                "passed": not turn_public["meta"]["review_required"] if case.behavior != EvalBehavior.UNCERTAINTY else True
            })
...
async def main():
    ...
    parser.add_argument("--fixtures", type=str, default="agent/app/evals/fixtures", help="Path to fixtures directory or file")
    parser.add_argument("--output", type=str, default="eval_results.json", help="Path to output JSON results")
    parser.add_argument("--db-url", type=str, default="postgresql+asyncpg://postgres:postgres@localhost:5433/app_scaffold_test", help="Database URL for orchestrator")
